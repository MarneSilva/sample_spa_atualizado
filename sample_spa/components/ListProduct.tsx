...
const ListProduct = ({}) => {
    const { Products, removeProduct, changeCategory } = useContext(ProductContext);
            ...
            <ul className="max-w-md space-y-1 text-gray-700 text-center mb-4">
                {Products.map((Product:Product, index:number) => (
                    <li className={`flex flex-center gap-2 mb-2 ${Product.category !== selectedTab ? 'hidden':''}`} key={index}>
                            <p className='space-x-2 space-y-2'>{Product.name}</p>
                            <p className='space-x-2 space-y-2'>{Product.preco}</p>
                            <p className='space-x-2 space-y-2'>{Product.description}</p>
                        <Select
                            options={options}
                            values={[options[Product.category]]}
                            valueField='value'
                            labelField='label'
                            onChange={(values) => changeCategory(index.toString(), values[0].value)}
                        />

                        
                        <button onClick={() => removeProduct(index.toString())}>
